            ...
            var setting = new EngineOptionsForSetting()
            {
                ...
                Descriptions = new List<EngineOptionDescription>()
                {
                    ...
                    new EngineOptionDescription("AutoThread_"           , "自動スレッド" ,
                        ...
                        "このオプションを有効にすると、思考の時に使用するスレッド数が、\r\n" +
            ...
            var setting = new EngineOptionsForSetting()
            {
                ...
                Descriptions = new List<EngineOptionDescription>()
                {
                    ...
                    new EngineOptionDescription("BookDir"      , null ,
                        ...
                        "思考エンジンの存在するフォルダからの相対パスで ../../book のように指定するか、\r\n" +
                        "c:/user/xxx のように絶対パスで指定するかします。前者が便利だと思います。\r\n" +
                    ...
                    new EngineOptionDescription("BookFile"      , "定跡ファイル" ,
                        ...
                        "ユーザー定跡1～3   : user_book1.db ～ user_book3.dbというファイル名の定跡ファイルを\r\n" +
                        "定跡フォルダに配置するとそれが使えます。(わかる人向け)\r\n" +
                        "Aperyの定跡ファイル : Aperyという将棋ソフトで使われている形式の定跡ファイルを使います。\r\n" +
                        "ファイル名は book.bin固定 です。(わかる人向け)",
                    ...
                    new EngineOptionDescription("BookOnTheFly"      , null ,
                        ...
                        "大きな定跡ファイルに対しては、対局開始時に丸読みしなくて済むので\r\n" +
                        "起動時間が短くなる効果と空き物理メモリが節約できる効果がありますが、"+
                        "そんな巨大な定跡ファイルを使うことはあまりないため、デフォルトではオフになっています。\r\n" +
                        "また、このオプションをオンにするには定跡ファイルがこのオプションに対応している必要があります。\r\n" +
                        "(このオプション非対応の定跡があります)\r\n" +
                    ...
                    new EngineOptionDescription("BookDepthLimit"      , null ,
                        "定跡の指し手のdepth下限",
                        "定跡に登録されている指し手の(定跡生成時の)depthがこれを下回るなら採用しない。\r\n" +
                        "0を指定するとdepth無視。\r\n" +
                    ...
                    new EngineOptionDescription("NarrowBook"      , null ,
                        "実現確率の低い定跡を採用しない",
                        "定跡ファイルの指し手に、出現頻度の情報がついている時に、\r\n" +
                        "出現頻度が低い指し手は採用しないためのオプション。",
                    ...
                    new EngineOptionDescription("MaxMovesToDraw"   , null ,
                        ...
                        "この項目を変更する時は、対局設定のほうの『指定手数で引き分けとする』も\r\n" +
                        "この設定に合わせて変更してください。\r\n"+
                    ...
                    new EngineOptionDescription("NetworkDelay"   , null ,
                        ...
                        "例えば、1手3秒で指す予定の時にこの値を350[ms]に設定した場合、2.65秒までに指し手を返します。",

                        "option name NetworkDelay type spin default 350 min 0 max 10000"),
                    ...
                    new EngineOptionDescription("NetworkDelay2"   , null ,
                        ...
                        "大きめの値(1000～1500)にすることを推奨します。",

                        "option name NetworkDelay2 type spin default 550 min 0 max 10000"),
                    ...
                    new EngineOptionDescription("MultiPV"   , null ,
                        "候補手の数を設定します",
                        "例えば、この項目(MultiPV)を5に設定すると、思考するときに候補手として5手挙げます。\r\n"+
                        "このとき、5手分調べないといけなくなるので、MultiPVを1に設定している時の\r\n" +
                        "約5倍の時間がかかるようになります。\r\n" +
                        "通常対局の時は1に設定しておくのが一番強くなります。\r\n" +
                        "また、検討モードの時は、この設定値は無視され、検討ウィンドウの『候補手』の数が反映されます。",
                    ...
                    new EngineOptionDescription("SlowMover"   , null ,
                        ...
                        "デフォルトは、100[%]。例えば、この値を70にすると序盤の1手に用いる時間が\r\n" +
                        "本来の時間の70%になり、序盤にかける時間が短くなります。",
                    ...
                    new EngineOptionDescription("ResignValue"   , null ,
                        ...
                        "例えば、相手からの詰みが見えた時に投了するのであれば、\r\n" +
                        "30000(詰みはこれより大きな値)と設定します。\r\n"+
                        "また、序盤で形勢に差がついた時に思考エンジンに投了させたいのであれば、\r\n" +
                        "1000～3000ぐらいの数値を設定すると良いでしょう。",
                    ...
                    new EngineOptionDescription("Contempt"   , null ,
                        ...
                        "千日手の局面を選ばずにそちらを選びます。\r\n" +
                        "デフォルトは2です。(互角に近いなら千日手の局面を回避して欲しいが、無理な打開はして欲しくないので。)",
                    ...
                    new EngineOptionDescription("NodesLimit"   , null ,
                        "探索ノード制限。",
                        "この値を0以外に設定すると、その値の探索ノード数(局面数)までしか探索しません。(弱くなります)\r\n" +
                        "強さを調整したい時に使います。",
                    ...
                    new EngineOptionDescription("EvalDir"   , null ,
                        ...
                        "評価関数用のファイルの配置するフォルダを変更する時にこの設定を使います。\r\n" +
                        "通常、変更する必要はありません。\r\n" +
                        "思考エンジンの存在するフォルダからの相対パスで ../../xxx_eval のように指定するか、\r\n" +
                        "c:/user/xxx のように絶対パスで指定するかします。前者が便利だと思います。\r\n" +
                        "詳しい人向けの設定であり、変更することはお勧めしません。",
                    ...
                    new EngineOptionDescription("OutputFailLHPV"   , null ,
                        ...
                        "それを調べ直している状態のことです。(fail lowはその逆)\r\n" +
